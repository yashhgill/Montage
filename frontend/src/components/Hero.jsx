...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const sync = () => { setIsMobile(media.matches); if (media.matches) setActive(0); };
    sync();
  ...
  useEffect(() => {
    ...
    if (reduceMotion || isMobile || heroSlides.length < 2) return;
  ...
  useEffect(() => {
    ...
    targets.forEach(({ el, end, suffix }) => {
      ...
      const timer = setInterval(() => {
        ...
        dom.textContent = Math.floor(start) + suffix;
      ...
      <div className="absolute inset-0">
        {slides.map((src, i) => (
          <div key={src} className={`hero-slide ${i === active ? "active" : ""}`}
            style={{ backgroundImage: `url('${src}')` }} aria-hidden="true" />
        ))}
      ...
      <div className="absolute bottom-20 right-6 md:right-12 z-10 hidden md:flex flex-col gap-3" aria-hidden="true">
        {heroSlides.map((_, i) => (
          <button key={i} onClick={() => setActive(i)}
            className={`block transition-all rounded-full ${i === active ? "w-2.5 h-8 bg-neon-cyan neon-glow-cyan" : "w-2 h-2 bg-white/30 hover:bg-white/60"}`}
            aria-label={`Go to slide ${i + 1}`} data-testid={`hero-dot-${i}`} />
        ))}
      </div>
