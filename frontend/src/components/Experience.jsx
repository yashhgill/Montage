...
function ExperienceCard({ item, i }) {
  ...
  return (
    <article
      ...
      className={`image-card-hover relative overflow-hidden rounded-2xl border border-white/10 bg-[#0A0A14] group reveal-item ${visible ? "revealed" : ""} ${item.large ? "col-span-2 row-span-2" : ""}`}
    ...
    >
      ...
      <div className="absolute inset-0 rounded-2xl border-2 border-neon-cyan/0 group-hover:border-neon-cyan/30 transition-all duration-500 pointer-events-none" />
      <div className="absolute inset-x-0 bottom-0 p-5 sm:p-6 translate-y-1 group-hover:translate-y-0 transition-transform duration-500">
        ...
        <p className="mt-2 text-xs sm:text-sm text-white/65 line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500">{item.caption}</p>
      </div>
    </article>
  );
}
  ...
  return (
    <section id="experience" data-testid="experience-section"
      ...
      <div className="relative max-w-7xl mx-auto">
        <div ref={headRef} className={`grid md:grid-cols-[0.35fr_0.65fr] gap-6 md:gap-10 items-end mb-12 reveal-item ${headVisible ? "revealed" : ""}`}>
          <p className="text-[11px] uppercase tracking-[0.32em] font-bold text-neon-yellow" data-testid="experience-kicker">Experience Zone</p>
