...
export default function AdminQuotationPage() {
  ...
  const [form, setForm] = useState({
    name: "", phone: "", email: "",
    bill_to_name: "", bill_to_address: "", valid_until: "",
    cc: "", bcc: "", remarks: "",
    venue: "", pax: "", notes: "",
  });
  ...
  const pickClientType = (type) => {
    setClientType(type);
    set({ bill_to_name: type === "corporate" ? form.bill_to_name : "", bill_to_address: "" });
  };
  ...
  const doPreview = async () => {
    ...
    setPreviewing(true);
    try {
      const { data } = await axios.post(`${API}/bookings/admin/preview-quotation`, {
        ...form, items: vi,
      }, authHeaders);
  ...
  const submit = async () => {
    ...
    if (!form.name.trim() && !form.bill_to_name.trim()) { setFormError("Customer name or Bill To name is required."); return; }
    if (!form.email.trim()) { setFormError("Customer email is required to send the quotation."); return; }
    const vi = validItems();
    if (vi.length === 0) { setFormError("Add at least one line item."); return; }
    setSubmitting(true);
    try {
      const { data } = await axios.post(`${API}/bookings/admin/quotation`, { ...form, items: vi }, authHeaders);
      setResult(data);
      if (data.ok) {
        setForm({ name:"",phone:"",email:"",bill_to_name:"",bill_to_address:"",valid_until:"",cc:"",bcc:"",remarks:"",venue:"",pax:"",notes:"" });
        setItems([blankItem()]);
    ...
    return (
      <div className="min-h-screen bg-[#050508] text-white grid place-items-center px-5">
        <div className="w-full max-w-sm">
          <p className="font-display font-black text-2xl text-center mb-6">MONTAGE<span className="text-neon-cyan">.</span> <span className="text-white/40 text-base font-normal">Quotations</span></p>
      ...
      <div className="border-b border-white/10 bg-black/60 backdrop-blur-xl sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-5 py-4 flex items-center gap-2">
          ...
          <span className="font-display font-black tracking-tight text-lg">
            ...
            <span className="text-white/40 text-sm font-normal">Create &amp; Send Quotation</span>
          </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 py-8 grid lg:grid-cols-2 gap-8">
        ...
        <div>
          <p className="text-white/55 text-sm mb-6">
            Send a professional quotation PDF to a client. Unlike an invoice, a quotation is an estimate — no payment or booking is created.
          </p>
          ...
          <Field label="Email (required — quotation will be sent here)" className="mt-4">
            <input value={form.email} onChange={(e) => set({ email: e.target.value })} type="email" placeholder="customer@email.com"
              className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm outline-none focus:border-neon-cyan" />
          </Field>
          <div className="grid sm:grid-cols-2 gap-4 mt-4">
            <Field label="Cc (optional)">
              <input value={form.cc} onChange={(e) => set({ cc: e.target.value })} placeholder="colleague@company.com"
                className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm outline-none focus:border-neon-cyan" />
            </Field>
          ...
          <p className="text-[11px] text-white/40 mt-1.5">jojo@montageevents.my is always Bcc'd automatically.</p>
          ...
          <Field label={clientType === "corporate" ? "Company Name" : "Bill To Name (optional)"}>
            <input value={form.bill_to_name} onChange={(e) => set({ bill_to_name: e.target.value })}
              placeholder={clientType === "corporate" ? "e.g. Hiranandani Hotels Sdn. Bhd." : "Leave blank to use customer name"}
              className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm outline-none focus:border-neon-cyan" />
          </Field>
          ...
          <Field label="Valid Until (optional)" className="mt-4">
            <input value={form.valid_until} onChange={(e) => set({ valid_until: e.target.value })} type="date"
              className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm outline-none focus:border-neon-cyan" />
          </Field>
          <p className="text-[11px] text-white/40 mt-1.5">Shown on the quotation as "Valid Until". Leave blank if no expiry.</p>
            ...
            {items.map((it, idx) => (
              <div key={idx} className="rounded-2xl border border-white/12 bg-white/[0.03] p-4">
                ...
                <input value={it.heading} onChange={(e) => setItem(idx, { heading: e.target.value })}
                  placeholder="Item heading, e.g. Bar & Beverages Package / Racing Simulator"
                ...
                <textarea value={it.details} onChange={(e) => setItem(idx, { details: e.target.value })} rows={4}
                  placeholder={"Use '- ' prefix for bullets:\nRacing Simulator\n- 1 unit, Logitech G29, 55inch LED TV\n- Suitable for all ages\n\nArcade Pro 32\n- 2500+ games, 32inch screen"}
                ...
                <p className="text-[10px] text-white/35 mb-3">Start a line with <span className="font-mono text-white/55">- </span> to make it a bullet. Leave a blank line between groups.</p>
          ...
          <div className="mt-5 flex justify-end text-sm">
            <span className="text-white/50 mr-3">Estimated Total:</span>
          ...
          <Field label="Remarks (optional)" className="mt-5">
            <textarea value={form.remarks} onChange={(e) => set({ remarks: e.target.value })} rows={3}
              placeholder={"e.g. Prices are subject to change based on final headcount.\nSetup time: 2 hours before event.\nTravel charges may apply outside Klang Valley."}
              className="w-full bg-white/[0.04] border border-white/12 rounded-xl px-4 py-3 text-sm outline-none focus:border-neon-cyan resize-none" />
          </Field>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3">
            <button onClick={doPreview} disabled={previewing}
              ...
              {previewing ? <><Loader2 size={18} className="animate-spin" /> Building preview…</> : <><Eye size={18} /> {previewUrl ? "Refresh Preview" : "Preview Quotation"}</>}
            </button>
            <button onClick={submit} disabled={submitting}
              ...
              {submitting ? <><Loader2 size={18} className="animate-spin" /> Sending…</> : <><Send size={18} /> Generate &amp; Send Quotation</>}
            </button>
          </div>

          {result && (
            <div className="mt-6 rounded-2xl border border-white/12 bg-white/[0.03] p-5 space-y-2 text-sm">
              <p className="font-bold text-white/90">Quotation No: <span className="text-neon-cyan">{result.quote_no}</span></p>
              <ResultRow ok={!!result.email_sent} label="Quotation emailed to customer" />
              {!result.email_sent && <p className="text-[11px] text-white/40">Email failed — check Gmail API credentials.</p>}
            </div>
          )}
        </div>
        ...
        <div className="lg:sticky lg:top-24 h-fit">
          ...
          <div className="rounded-2xl border border-white/12 overflow-hidden bg-[#0A0A14] h-[75vh]">
            {previewUrl ? (
              <iframe title="Quotation preview" src={previewUrl} className="w-full h-full bg-white border-0" />
            ) : (
              <div className="h-full grid place-items-center text-white/35 text-sm px-8 text-center">
                Fill in the form and click <span className="text-neon-cyan font-semibold mx-1">Preview Quotation</span> to see it here. Nothing is sent until you click Generate &amp; Send.
              </div>
            )}
          </div>
        </div>
